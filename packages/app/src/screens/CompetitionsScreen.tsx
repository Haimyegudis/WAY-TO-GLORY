...
import { club, myCompetitionState, table, topScorers } from '../state/selectors.js';
import { Card, ClubLine, Crest, Empty } from '../components/ui.js';

type View = 'table' | 'scorers' | 'europe';

/**
 * The competitions, as opposed to the club.
 *
 * Tables, scoring charts and European nights are football he follows, not business he
 * has with his own club - and putting them behind the club badge meant the club screen
 * was mostly other people's leagues. His employer is one tab; the game he plays in is
 * another.
 */
export function CompetitionsScreen() {
  ...
  const inEurope = Object.keys(state.world.europe ?? {}).length > 0;

  return (
    <div className="screen stack">
      <header>
        <p className="eyebrow">{t('nav.competitions')}</p>
        <h1 className="title">{t('nav.competitions.title')}</h1>
      </header>

      <div className="seg">
        <button aria-pressed={view === 'table'} onClick={() => setView('table')}>{t('club.table')}</button>
        <button aria-pressed={view === 'scorers'} onClick={() => setView('scorers')}>{t('club.scorers')}</button>
        {inEurope && (
          <button aria-pressed={view === 'europe'} onClick={() => setView('europe')}>{t('competition.europe')}</button>
        )}
      </div>

      {view === 'table' && <LeagueTable />}
      {view === 'scorers' && <Scorers />}
      {view === 'europe' && inEurope && <EuropeanRun />}
    </div>
  );
}
